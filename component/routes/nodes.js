...
router.put('/list', (req, res, next) => {
  const {id, newName} = req.body
    if(id){
      const result = renameNode(id, newName)
...
router.put('/status',(req, res, next) => {
  const {id, maxCurrent, workmode, workStatus } = req.body
    if(id){
      const result = changeNodeSetting(id, maxCurrent, workmode, workStatus)
      return result.then(val => {
          if (val) {
            return  res.json( new SuccessModel({'msg': 'successfully change the setting the node', 'status': 202})
          )}
          return res.json(
            new ErrorModel({'msg': 'Failed to rename the node because id does not exist', 'status': 404})
          )
      })
    }
    return res.json(
